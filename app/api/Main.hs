...
import Eclogues.Scheduling.Command ( ScheduleConf (ScheduleConf)
                                   , runScheduleCommand, schedulerJobUI )
...
mkOutputURI :: ApiConfig -> Job.Name -> FilePath -> String
...
withPersist mkConf conf webLock followAuroraFailure = do
    (_, st) <- ES.runStateTS def $ do
        (js, cmds) <- lift . Persist.atomically (Config.pctx conf) $ do
            js <- Persist.allJobs
    ...
    let web = Lock.with webLock $ serve 8000 conf stateV
        updater = forever $ do
            loadSchedulerState mkConf conf stateV
            threadDelay . floor $ second (1 :: Double) `asVal` micro second
        enacter = forever . STM.atomically $ do -- TODO: catch run error and reschedule
            cmd <- readTChan $ Config.schedChan conf
            auri <- Config.requireAurora conf
